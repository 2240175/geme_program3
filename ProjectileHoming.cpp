...
void ProjectileHoming::Update(float elapsedTime)
{
	...
	//旋回
	{
		...
		//ターゲットまでのベクトルを算出
		DirectX::XMVECTOR Position = DirectX::XMLoadFloat3(&position);//玉の位置(position)をXMVECTORに変換
		DirectX::XMVECTOR Target = DirectX::XMLoadFloat3(&target);//ターゲット(target)をXMVECTORに変換
		DirectX::XMVECTOR Vec = DirectX::XMVectorSubtract(Target, Position); //ターゲット-玉の位置
		//ゼロベクトルでないなら回転処理
